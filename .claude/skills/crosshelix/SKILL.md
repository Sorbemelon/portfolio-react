---
name: crosshelix
description: Use CrossHelix for non-trivial Claude Code repo-integrity work — current local evidence, precise repo-term retrieval, docs/spec sync support, responsibility/impact/handoff signals, and safe memory — without replacing Read/Edit/Grep or tests. Skip trivial, formatting-only, already-open one-file edits, and non-repo work.
---
<!-- BEGIN CROSSHELIX MANAGED BLOCK -->
# CrossHelix for Claude Code

Use this skill when Claude Code needs local repo-integrity evidence. Keep `CLAUDE.md` short; this skill carries the full workflow. CrossHelix is a local, deterministic evidence layer for repo structure, indexed files, symbols, map links, sessions, decisions, notes, feedback, responsibility areas, impact, freshness, and handoff memory. Treat every result as evidence, not truth.

## Claude Code tool boundary

CrossHelix returns repo evidence and recorded memory. Claude Code still uses Read, Edit, Grep, terminal commands, and project tests for live inspection and edits. CrossHelix never runs Claude’s tools, never edits source, never calls LLM/cloud/vector services by default, never performs Git merge/rebase/switch/commit/push, does not guarantee correctness, and does not replace human review or tests.

If a local `UserPromptSubmit` reminder hook is installed, treat it as a reminder only. It must not force CrossHelix on trivial or non-repo tasks, and it does not block tools, change files, or run workflow commands.
...
crosshelix prepare "auth login handler" --agent claude
```
...
crosshelix prepare "AuthService validateSession tests" --agent claude
```
...
3. Use `crosshelix search "<repo terms>"` for targeted lookup, or `crosshelix prepare "<short task terms>" --agent claude` for a focused implementation pack.
4. Use Read/Grep before editing. Indexed content is not a substitute for live files.
...
10. Run project tests, then `crosshelix guardcheck` before asking for acceptance on meaningful work.
...
4. `prepare "<short docs/spec sync terms>" --agent claude` only after the terms are precise.
...
```text
crosshelix log-usage --agent claude --files "src/services/auth_service.py,tests/test_auth_service.py" --note "Used service and test context."
crosshelix session update --agent claude --task "auth service validation" --status completed --files "src/services/auth_service.py,tests/test_auth_service.py" --note "Validated auth service behavior."
crosshelix note add --agent claude --type handoff --text "Validation complete; review token expiry edge cases next." --files "src/services/auth_service.py"
...
## Claude reminder behavior

For non-trivial repo tasks, decide whether CrossHelix is useful. If useful, formulate precise repo terms first and check index freshness. If not useful, skip CrossHelix and continue normally with Read/Edit/Grep and project tests.
...
crosshelix prepare "auth service validation" --agent claude
...
crosshelix handoff
crosshelix session update --agent claude --task "auth service validation" --status in_progress --files "src/services/auth_service.py" --note "Reviewing validation flow."
crosshelix log-usage --agent claude --files "src/services/auth_service.py" --note "Used service context."
